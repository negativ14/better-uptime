        ...
        <footer className="border-t border-slate-800/40">
            <Container>
                <div className="flex flex-col lg:flex-row justify-between gap-12 py-14">
                    <div className="flex flex-col gap-4">
                        <a href="/" className="flex items-center gap-2.5">
                            <div className="w-7 h-7 rounded-md bg-gradient-to-br from-sky-500 to-cyan-400 flex items-center justify-center">
                                <Activity className="w-3.5 h-3.5 text-white" />
                            </div>
                            <span className="text-2xl font-display font-bold tracking-tight text-white">BetterUptime</span>
                        </a>
                        ...
                        <div className="flex gap-2">
                            {[Twitter, Linkedin, Github].map((Icon, i) => (
                                <a key={i} href="#" className="w-8 h-8 rounded-md p-2 border border-slate-800/60 bg-slate-900/40 flex items-center justify-center text-slate-600 hover:text-white hover:border-slate-700 transition-all">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between py-5 border-t border-slate-800/30 gap-3">
                    <p className="text-sm text-slate-700">© 2026 BetterUptime. All rights reserved.</p>
                    <div className="flex items-center gap-5">
                        {["Privacy", "Terms", "Cookies"].map((item) => (
                            <a key={item} href="#" className="text-sm text-slate-700 hover:text-slate-400 transition-colors">{item}</a>
                        ))}
                    </div>
                </div>
            </Container>
